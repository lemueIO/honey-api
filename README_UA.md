...
-   **Розумна фільтрація та очищення**:
    -   **Білий/Чорний список**: Підтримує точні збіги IP та діапазони CIDR (наприклад, `10.0.0.0/24`).
    -   **Високопродуктивне очищення**: Оптимізоване видалення застарілих даних за допомогою сканування заздалегідь підготовленого чорного списку.
-   **Розширене логування**:
    -   **Структуровані логи**: Повністю оновлена система з кольоровими ANSI-тегами (`[SYSTEM]`, `[CLEAN:DB]`, `[FETCH:OSINT]`) для максимальної чіткості.
    -   **Візуальний відгук**: Інтегрований жовтий ASCII-логотип при запуску та з періодичністю 12 годин.
-   **Надійний моніторинг**:
